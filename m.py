...
bot = telebot.TeleBot('7535112403:AAFWR3Y-Tulvjs-FgNydykZhOF-6WiPgH6k')
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "O OO Comand Not Work Because its not Free Dm To Purchase @Reo_47 ❄."
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, 𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃 🚀🚀\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: VIP- @Reo_47 Ddos Service"
    bot.reply_to(message, response)
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = ("🚫 Unauthorized Access! 🚫\n\nOops! It seems like you don't have permission to use the /bgmi command. DM TO BUY ACCESS:- @Reo_47")
...
🧑🏻‍💻 /rules: Read usage rules.
🧑🏻‍💻 /mylogs: View recent attacks.
🧑🏻‍💻 /plan: Check botnet rates.
...
def welcome_rules(message):
    user_name = message.from_user.first_name
    response = f'''{user_name} ⚠️ Rules ⚠️:

1. Limit the number of attacks to avoid a ban.
2. Don’t run two attacks simultaneously; it will lead to a ban.
3. Ensure you’ve joined @REOHECKS, or it won’t work.
4. Follow these rules; logs are checked daily.'''
    bot.reply_to(message, response)
